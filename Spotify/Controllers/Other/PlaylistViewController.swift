...
class PlaylistViewController: UIViewController {
            ...
            DispatchQueue.main.async {
                ...
                case .success(let model):
                    ...
                    self?.viewModels = model.tracks.items.compactMap({
                        RecommendedTrackCellViewModel(trackName: $0.track.name,
                                                      artistName: $0.track.artists.first?.name ?? "N/A",
                                                      artworkURL: URL(string: $0.track.album?.images.first?.url ?? "N/A"))
                    })
    ...
    @objc func didLongPress(_ gesture: UILongPressGestureRecognizer){
        ...
        let actionSheet = UIAlertController(title: trackToDelete.name, message: "Are you sure you want to remove this track?", preferredStyle: .actionSheet)
            ...
            APICaller.shared.removeTrackFromPlaylist(track: trackToDelete, playlist: strongSelf.playlist) { success in
                DispatchQueue.main.async {
                    if success {
                        ...
                        print("Failed to remove the track")
                    }
                }
